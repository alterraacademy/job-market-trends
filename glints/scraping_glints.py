...
# list_of_jobs = {'software engineer','backend developer','frontend developer','mobile developer','quality engineer','data analyst','data scientist'}
# searched_position = input('Enter the job you want to search: ')
# searched_position = searched_position.replace(' ', '+')

# for searched_job in list_of_jobs:
...
while True:
    ...
    for job_card in job_cards:
        ...
        job_tags = soup.find_all('div',class_='TagStyle__TagContentWrapper-sc-r1wv7a-1 koGVuk')
        work_place = job_tags[0].text if len(job_tags) > 0 else None
        years_experience = job_tags[2].text if len(job_tags) > 2 else None
        min_education = job_tags[3].text if len(job_tags) > 3 else None
